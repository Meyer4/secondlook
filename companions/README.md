...
- **Android:** `secondlook-android-preview.apk` is a non-debuggable, preview-signed build, not a Play Store release. Review permissions and use a test device first. Do not disable Play Protect or Android’s sensitive-content protections. Version 1.3 installs as a separate app that shows as SecondLook (its own package identity). Configure it again, and pause the older installation to avoid duplicate warnings. Owner-controlled production signing is still required for a normal long-term update path.
